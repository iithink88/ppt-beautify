...
- **`--cover hero` 封面大图模式**：递归找封面最大图片 → 删除该页所有图片/分组/全屏块 → 用原图数据重铺一张全屏 hero → 叠 55% 透明深色蒙版 → 标题文字置顶。仅改输出文件，原稿不动。
...
- **封面大图模式**：`--cover hero` 已内置。它会清掉封面所有拼贴图/分组，只留最大一张铺满全屏（轻微拉伸属正常，想要不变形可后续手动裁切）。若标题原本在分组里（少见），可能被一并清掉，需手动补。
